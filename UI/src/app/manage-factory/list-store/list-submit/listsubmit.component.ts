...
export class ListSubmitFSComponent implements OnInit {
        ...
        if(id){
          ...
          .subscribe({
            next: (re) => {
              ...
              this.testService.getAllFactoryRequestbyFactorys(re.workingFor,'Đã xác nhận',"Last 7 Day_Đăng")
                .subscribe({
                  next: (f)=>{
  ...
  filter(req: string, id: string) {
    this.testService.getAllFactoryRequestbyFactorys(this.factoryId, 'Đã xác nhận', req +"_"+ id)
      .subscribe({
        next: (f) => {
          this.dataSource = new MatTableDataSource(f);
          this.dataSource.paginator = this.paginator;
