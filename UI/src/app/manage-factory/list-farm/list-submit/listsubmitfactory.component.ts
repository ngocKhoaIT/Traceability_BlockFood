...
@Component({
  selector: 'app-listsubmitfactory',
  templateUrl: './listsubmitfactory.component.html',
  styleUrls: ['./listsubmitfactory.component.scss'],
  providers: [
    {
...
export class ListSubmitFactoryComponent implements OnInit {
  ...
  choices: string[] = ["Đăng", "Đang tìm" ,"Đang vận chuyển", "Hoàn tất"]
  choice = ''
  ...
  transfer(id: string, h: string, f: string, w: number, u: string) {
    const dialogRef = this.dialog.open(popUpDetailFAS, {
      width: '700px',
      height: 'auto',
...
export class popUpDetailFAS implements OnInit {
  constructor(
    public dialogRef: MatDialogRef<popUpDetailFAS>,
          ...
          this.testService.updateStatusBnt(this.addDetail.goodsId, this.addDetail.toPlace, 'Đang tìm')
            .subscribe({
              next: (re) => {
                ...
                this.addNotice.title = "Đăng tìm vận chuyển"
                this.addNotice.content = "Tìm vận chuyển từ nông trại tới nhà máy"
