...
namespace TraceabilityAPI.Repositorys.Repository
{
    ...
    { 
        ...
        {
            var istore = context.Inventories.Where(t => t._status == 1 && t.status_request == req && t.storeId == id).OrderByDescending(t=>t.date_update);
        ...
        {
            var istore = context.Inventories.Where(t =>t._status == 1 && t.status_request == "Đã nhận").ToList();
            ...
            List<IL> result = ils.Where(t=>t.storeId == id).GroupBy(t => t.goodsName)
                            .Select(t1 => new IL
                            {
                                id = t1.First().id,
        ...
        public JsonResult detailProductFull2(string id)
        {
            var istore = context.Inventories.Where(t => t.id == id && t._status == 1).ToList();
            var store = context.Stores.Where(t => t._status == 1).ToList();
            var ifactory = context.InventoryFactories.Where(t => t._status == 1).ToList();
