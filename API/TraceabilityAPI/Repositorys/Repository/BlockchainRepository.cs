    ...
    public class BlockchainRepository : BaseRepository<Blockchain>, IBlock
    {
        ...
        public string AddDataBlock()
        {
            ...
            foreach (var block in inventory)
            {
                ...
                {
                    data += JsonConvert.SerializeObject(iven.detailProductWTM(block.id).Value) + "_";
                }
                else
                if(iven.detailProductLiveFull(block.id) != null && JsonConvert.SerializeObject(iven.detailProductLiveFull(block.id).Value) != "[]")
                {
                    data += JsonConvert.SerializeObject(iven.detailProductLiveFull(block.id).Value) + "_";
                }
                else
                {
                    data += JsonConvert.SerializeObject(iven.detailProductLiveWTM(block.id).Value) + "_";
                }
            }
            return data;
        }
